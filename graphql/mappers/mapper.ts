...
export class Mapper {
  ...
  public peopleFromResponse(personGrpc: PersonGrpc[]): PersonGraphql[] {
    return personGrpc
      .map((p) => PersonGrpc.toObject(true, p))
      .map((pp) => this.person(pp));
  }

  public house(houseGrpc: HouseGrpc.AsObject): HouseGraphql {
    return {
  ...
  public houseFromResponse(houseGrpc: HouseGrpc[]): HouseGraphql[] {
    return houseGrpc
      .map((h) => HouseGrpc.toObject(true, h))
